...
import { queryKeys, postComment, isRequired, isEmailFormat } from "@richie/lib";
import { Button, Input, Textarea } from "@richie/components";
...
export type CommentBody = Pick<Comment, "postId" | "name" | "email" | "body">;
...
export default function CommentComposer({ postId }: { postId: number }) {
	...
	const {
		...
		mutationFn: async (newComment: CommentBody) => {
			return await postComment<Comment, CommentBody>(newComment);
		},
		onSuccess: data => {
